...
// Helper function to get score band info based on percentage
...
// Circular progress component for score with band label
function ScoreCircle({ score }: { score: number }) {
  ...
  return (
    <div className="flex flex-col items-center gap-1.5">
      <div className={cn("relative flex items-center justify-center rounded-full", band.bg)} style={{ width: 72, height: 72 }}>
        ...
        <span className={cn("text-sm font-bold z-10", band.text)}>{score}%</span>
      </div>
      <span className={cn("text-xs font-semibold px-2 py-0.5 rounded-full", band.labelBg, band.labelText)}>
        {band.label}
      </span>
    </div>
  );
...
export default function AssessmentCard({
  ...
  const cardContent = (
    <>
      <div className="p-3 w-full flex-1">
        <div className="flex items-start justify-between w-full gap-3">
          <div className="flex-1">
            ...
            <h3 className="font-semibold leading-normal text-black text-lg mt-4">
              {title}
            </h3>
          ...
          {/* Score Circle - Prominent display for taken assessments */}
          {isTaken && (
      ...
      <div className="flex flex-col items-start p-3 w-full mt-auto">
